...
export const sendVerificationEmail = async (user, res) => {
  ...
  const link =  CLIENT_URL+ "verified/" + _id + "/" + token;
...
export const resetPasswordLink = async (user, res,token) => {
  const { _id, email } = user;
  const link = CLIENT_URL  + "change-password/" + _id + "/" + token;
