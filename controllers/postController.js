  ...
  try {
    ...
    if (index === -1) {
      ...
      const newNotification =new Notification({
        ...
        postImg:post.image || likedUser.profileUrl,
      })
...
export const commentPost = async (req, res, next) => {
  try {
    ...
    const postOwner=await Users.findById(post.userId);
    const commentFrom =await Users.findById(userId)

      const newNotification =new Notification({
        from:userId,
        to:post.userId,
        postId:id,
        message:`${commentFrom.firstName} ${commentFrom.lastName} commented on your post`,
        postImg:post.image,
      })
      await newNotification.save()
